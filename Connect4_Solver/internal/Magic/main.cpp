...
int main(int argc, char** argv) {
  ...
  bool weak = false;
  if(argc > 2 && argv[2][0] == '-' && argv[2][1] == 'w') weak = true;

  std::string line = argv[1];

  Position P;
  if(P.play(line) != line.size()) {
    // Test if last move will win
    Position PTestWin;
    int lastMove = int(line[line.size() - 1] - '1');
    line.pop_back();
    PTestWin.play(line);
    if(PTestWin.my_canPlay(lastMove) && PTestWin.my_isWinningMove(lastMove)) {
      std::cout << 999999;
    }
    else {
